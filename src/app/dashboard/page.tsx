...
const Dashboard = () => {
  ...
  const [chnage, setsetChange] = useState<boolean>(false);
            ...
            <div className="flex-1 w-full grid gap-4 grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
              {
                !loading && courses?.length ? (
                  courses.map(course => (
                    <Card key={course.id} id={course.id} title={course.title} description={course.description} setChnage={setsetChange} />
                  )) 
                ) : <></>
              }
            </div>
